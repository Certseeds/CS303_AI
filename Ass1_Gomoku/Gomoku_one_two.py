...
def judgement_doubles(align,color):
    ...
    for i in align:
        ...
        for j in range(0,len(i)-4):
            # 活二 1
            # print(i[j:j+5])
            # print(np.asarray([0,0,color,color,0]))
            if np.array_equal(i[j:j+5],[0,0,color,color,0]):
                net_score += 5
                multiply_live_two += 1
            elif np.array_equal(i[j:j+5], [0,color,color,0,0]):
                net_score += 5
                multiply_live_two += 1
            elif np.array_equal(i[j:j + 5], [0, color,  0, color,0]):
                net_score += 5
                multiply_live_two += 1
            # 眠二 1
            if np.array_equal(i[j:j + 5] ,[color, color,  0, 0,color]) or np.array_equal(i[j:j+5],[color,0,0,color,color]):
                net_score += 3
